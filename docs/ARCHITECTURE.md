...
- Gmail API: Polling (5 Min Intervall via Cron Job). Pub/Sub spaeter wenn noetig
- Thread-Management: 3-stufig — Gmail Thread-ID (primaer) → Subject-Parsing (Kommission/Adresse) → Manuelle Zuordnung via Telegram (Fallback)
- E-Mail-Routing: Phase 1 alles ueber E&W. Spaeter Direktmodus als Feature-Flag
- Uebersetzung: Claude Sonnet mit TERMINOLOGY.md als System-Kontext
- Spracherkennung: Claude erkennt DE/PL im Uebersetzungs-Call. Judyta wird informiert nur noch PL zu schreiben
- Human-in-the-Loop: Bridge erstellt Gmail-Draft → Telegram-Notification mit Preview + Inline Keyboard (Freigeben/Bearbeiten/Ablehnen) → Bei Freigabe sendet Bridge den Draft automatisch
- E-Mail-Status-Flow: RECEIVED → DRAFT → APPROVED_BY_EW → SENT (oder REJECTED)
- Attachments: Durchreichen ohne Verarbeitung (Phase 2)
- Telegram: Shared Bot mit Clara Voice, Bridge-Messages mit Prefix "[Bridge]"
